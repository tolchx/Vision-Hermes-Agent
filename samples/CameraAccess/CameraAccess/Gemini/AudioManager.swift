...
class AudioManager {
  ...
  @objc private func handleBackground() {
    NSLog("[Audio] App entered background — keeping audio session alive")
    // Ensure audio session stays active
    try? AVAudioSession.sharedInstance().setActive(true, options: .notifyOthersOnDeactivation)
  }

  @objc private func handleForeground() {
    NSLog("[Audio] App returned to foreground — restarting audio if needed")
  ...
  func setupAudioSession(useIPhoneMode: Bool = false) throws {
    ...
    try session.setCategory(
      .playAndRecord,
      mode: mode,
      options: [.defaultToSpeaker, .allowBluetoothHFP, .allowAirPlay, .allowBluetoothA2DP]
    )
    try session.setPreferredSampleRate(GeminiConfig.inputAudioSampleRate)
    try session.setPreferredIOBufferDuration(0.064)
    try session.setActive(true, options: .notifyOthersOnDeactivation)
    
    // Configure NowPlaying for background audio control
    setupNowPlaying()
    
    NSLog("[Audio] Session mode: %@ (background audio enabled)", useIPhoneMode ? "voiceChat (iPhone)" : "videoChat (glasses)")
  }
  
  private func setupNowPlaying() {
    let center = MPNowPlayingInfoCenter.default()
    var info = [String: Any]()
    info[MPMediaItemPropertyTitle] = "VisionHermes"
    info[MPMediaItemPropertyArtist] = "AI Assistant"
    info[MPNowPlayingInfoPropertyPlaybackRate] = 1.0
    center.nowPlayingInfo = info
    
    // Handle remote control events (play/pause from lock screen)
